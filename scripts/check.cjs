...
(async () => {
  // colección, revista y álbum
  {
    ...
    assert.equal(d.querySelectorAll('#tabs button').length, new Set(GAMES.map(g => g.model)).size, 'una página del álbum por modelo');
    assert.ok(d.querySelectorAll('#spread .cslot').length > 0, 'huecos vacíos al empezar'); assert.equal(d.querySelector('#pack-btn').disabled, true, 'sin sobres al empezar');
